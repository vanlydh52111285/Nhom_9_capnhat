...
namespace nhom_9
{
    ...
    public partial class frm_dat_lich : Form
    {
        ...
        private void ThoiGianThongBao_DropDown(object sender, EventArgs e)
        {
            ThoiGianThongBao.Items.Clear();
            ThoiGianThongBao.Items.Add("trước 15 phút");
            ThoiGianThongBao.Items.Add("trước 30 phút");
            ThoiGianThongBao.Items.Add("trước 1 tiếng");
        }
        ...
        private void NotifyTime(DatLich dl)
        {
          
            DateTime thoilamviec = dl.thoigian;
            DateTime thoigianhientai = DateTime.Now;
            TimeSpan thoigianconlai = thoigianhientai - thoilamviec;
            if (thoigianconlai.TotalMinutes <= notifyTime)
            {
                MessageBox.Show("Sự kiện sắp diễn ra trong " + thoigianconlai.TotalMinutes + " phút.");
            }
        }
        ...
        private void SoLanThongBao_DropDown(object sender, EventArgs e)
        {
            SoLanThongBao.Items.Add("1 lần/Ngày");
            SoLanThongBao.Items.Add("1 lần/Tuần");
        }

        private void SoLanThongBao_SelectedIndexChanged(object sender, EventArgs e)
        {
            switch (SoLanThongBao.SelectedIndex)
            {
                case 0:
                    repeatTime = 1;
                    break;
            ...
            MessageBox.Show("Đã cài đặt thông báo " + SoLanThongBao.SelectedItem);
        }
        private void RepeatTime(DatLich dl)
        {
            DateTime now = DateTime.Now;
            if ((now - dl.thoigian).TotalDays >= repeatTime)
            {
                MessageBox.Show("Đây là thông báo nhắc nhở công việc của bạn", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            lastNotify = now;
        }
